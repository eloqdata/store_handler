...
bool DataStoreServiceClient::AppendPreBuiltTable(
    ...
    const std::string &backup_name, std::vector<std::string> &snapshot_files)
{
    ...
    while (need_retry && retry_count < retry_limit_)
    {
        ...
        if (IsLocalShard(req_shard_id))
        {
            ...
            bool done = false;
            EloqDS::remote::CreateSnapshotRequest req;
            req.set_backup_name(backup_name);
            req.set_req_shard_id(req_shard_id);

            data_store_service_->CreateSnapshotForBackup(
                req,
                [this, &ret, &need_retry, &snapshot_files, &mtx, &cv, &done](
                    const ::EloqDS::remote::CreateSnapshotResponse &resp)
                {
                    ...
                    {
                        LOG(ERROR)
                            << "CreateSnapshotForBackup failed, error code: "
                    ...
                    else
                    {
                        ...
                        for (const auto &file : resp.snapshot_files())
                        {
        ...
        {
            ret = CallRpcCreateSnapshotForBackup(
                backup_name, snapshot_files, need_retry);
        }

        if (need_retry)
        {
            LOG(ERROR) << "CreateSnapshotForBackup failed, retrying...";
            retry_count++;
        }
    }
...
{
    // We don't need to implement this function, because the backup snapshot is
    // on S3 rather than local, and their lifecycle is managed by another
    // purging service.
    return true;
}

bool DataStoreServiceClient::RemoveBackupSnapshot(
...
bool DataStoreServiceClient::CallRpcCreateSnapshotForBackup(
    const std::string &backup_name,
    std::vector<std::string> &snapshot_files,
    bool &need_retry)
{
    ...
    stub.CreateSnapshotForBackup(&cntl, &req, &resp, nullptr);
    if (cntl.Failed())
    {
        auto &result = resp.result();
        LOG(ERROR) << "CreateSnapshotForBackup RPC failed, result:"
    ...
    {
        LOG(ERROR) << "CreateSnapshotForBackup failed, result:"
                   << static_cast<int>(result.error_code())
...
{
    while (!ds_serv_shutdown_indicator_.load(std::memory_order_acquire))
    {
        std::unique_lock<bthread::Mutex> lk(ds_service_mutex_);
